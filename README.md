...
아티스트 이름 맞추기 게임을 즐길 수 있는 Electron 데스크탑 앱입니다.
...
| 특별 효과     | 영어 아티스트 입력 시 단어 정지 또는 시간 +5초 효과 |
...
화면 위에서 아티스트 이름이 떨어지면 타이핑해서 없애세요. 라운드가 올라갈수록 속도와 단어 수가 증가하고, 영어 아티스트를 맞추면 단어 정지 또는 남은 시간 +5초 특별 효과가 발동됩니다. Google 로그인 시 점수가 자동 저장되고 전체 랭킹을 확인할 수 있어요.
